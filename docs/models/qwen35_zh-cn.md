...
| MTP 投机解码 | Qwen 3.6 —— NextN 草稿块内嵌在主干 GGUF 中（无需独立文件；仅限保留 MTP 的 GGUF，见[下载](#下载)）；用服务端标志 `--mtp-spec` 启用。部分接受时做 GDN 递归状态快照 / 回滚。只要 GGUF 保留 NextN 块，就会对单序列（无并发）请求启用。详见 §12。 |
| 输出解析器 | `Qwen35OutputParser`（继承 `Qwen3OutputParser`） |

## 下载

已验证的 GGUF 来源（下表中每个模型使用独立目录 —— 9B 与 3.6 两个仓库的投影器
都叫 `mmproj-F16.gguf`，因此不能共用同一目录）：
...
> 中的文件（文件名相同）去掉了该块 —— 对这些文件，`--mtp-spec` 会静默回退到
> 标准 decode。
...
CLI 单次推理（文本提示词通过 `--input` 从文件读取；采样默认为 greedy，
`--max-tokens` 默认为 100 —— 实际问答请调大）：
...
服务端 MTP 演示（`--mtp-spec` 与其他 `--mtp-*` 标志**只存在于
TensorSharp.Server** —— `TensorSharp.Cli` 没有 MTP 标志；在 CLI 上只能通过
`TS_MTP_*` 环境变量启用 MTP）：
...
### 融合 QK-Norm + RoPE（direct CUDA）

在 direct `cuda` 后端上，纯文本 prefill 路径把 per-head QK RMSNorm 与 NeoX RoPE
融合为一个 CUDA kernel（`ts_qk_norm_rope_neox_f32`）。默认开启；设
`TS_FUSED_QKNORM_ROPE=0` 可回退到独立的 Norm + RoPE 算子（多模态 MRoPE 与非
CUDA 后端始终走独立路径）。
...
Qwen 3.6 的 GGUF 可以自带一个 **NextN / 多 token 预测（MTP）草稿块**，`TensorSharp.Server`
...
文件中，因此不使用 `--mtp-draft-model`（若显式传入一个无法在启动模型上激活的
草稿模型，会在启动时快速失败报错）。
...
投机默认关闭，用**服务端**标志 `--mtp-spec`（环境变量 `TS_MTP_SPEC=1`；
`TensorSharp.Cli` 没有 MTP 标志）启用。只要加载的 GGUF 保留 NextN 块，它就会对
单序列（无并发）请求启用；对不含该块的 GGUF，引擎会静默走标准 decode。
`--mtp-draft`（默认 `8`）限制草稿窗口，
