...
draft / verify / rollback core — Gemma 4 is the `draft-head` algorithm of the
pluggable `--spec-type` layer, which `auto` already picks from the checkpoint.
