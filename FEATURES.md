...
- **Code execution** -- with `--code-exec` the model drives a real shell in a sandboxed workspace, typing a command line and reading back the exit code and everything it printed. The workspace persists for the whole chat session and is shared with skill scripts, so `cd`, exported variables, installed packages and files written earlier all survive to the next call; file work goes through a dedicated surface rather than through the shell: `read_file` shows a file's real bytes with line numbers, `edit_file` replaces one exact string in one file, `write_file` creates a file, and `apply_patch` changes several files at once, all or nothing. That is the shape both references use — Claude Code's `Read`/`Edit`/`Write` for the common case and Codex's `apply_patch` envelope for the atomic multi-file one — and it exists because a heredoc re-emits the *whole* file, so a one-line fix costs every line that was already right and re-rolls each of them. The host places the bytes, from text it either finds exactly or refuses to guess at. Off by default, and it needs a real OS sandbox (`sandbox-exec` on macOS, `bwrap` on Linux) — without one the tool refuses rather than running unconfined.
...
## DFlash / DFlash2 Block Speculative Decoding (Muse-Glimmer, Qwen 3.8)
...
| `block` | a whole block per pass with a confidence head (DeepSeek V4 DSpark, DFlash / DFlash2 on Muse-Glimmer and Qwen 3.8) | yes, per target model |
...
effect that keeps `TS_BATCHED_FUSED_DECODE` off by default -- but it is **not**
...
|---|---|
| Qwen 3, Mistral 3, Gemma 3/4, Qwen 3.5/3.6-family, GPT OSS, Nemotron-H, Muse-Glimmer (`--tp 2` max) | Tensor parallelism, opt in with `--tp N` |
| GLM-5.2 (`glm-dsa`) | Layer split across every visible GPU by default; `--tp N` switches it to tensor parallelism |
| GLM-5.3-Flash (`glm5next`) | Layer split across every visible GPU by default; `--tp` is refused cleanly |
...
**Script sandbox.** When script execution is enabled, the child runs under `sandbox-exec` on macOS and `bwrap` on Linux — network denied, the user's home directory unreadable, writes confined to a per-run scratch directory — plus, on every platform, an interpreter allow-list, no shell, a scrubbed environment that withholds host credentials, a time limit and an output cap. Windows bounds the process tree through a job object but cannot confine the filesystem or network, and says so: every result names what was *not* enforced. `--skills-sandbox required` (the default) refuses to run scripts on a host that cannot confine them, rather than running them unconfined.

**Model families.** Gemma 3 and Mistral 3 chat formats carry no tool declarations, so on those the selected skill bodies are written into the prompt up front and `skills_read` is not offered; Mistral 3 additionally drops `role: "tool"` messages, so results are fed back as a user turn.
