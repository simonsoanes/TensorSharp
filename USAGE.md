...
rises from 8 to 24 when this is on. Web/CLI keeps the working directory for the
whole chat session; each OpenAI/Ollama request keeps a private directory across
its internal tool rounds and deletes it afterward. With code execution enabled,
skill scripts share that workspace, so `cd`, exported variables, an activated
virtualenv and installed packages survive for the applicable session or request.
There is no long-lived shell behind that: a Seatbelt profile is fixed
