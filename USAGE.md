...
| `--code-exec-allow-install` | Let the model install the packages it needs (pip / npm) into an environment the session keeps, so a later command — and a skill's own scripts — can import them too; requires `--code-exec`. The command the model typed still gets **no network**: the host does not run its install, it *reads* it — the tool and the package names, nothing else — checks the names, and performs the install itself with an argument vector it built, wheels only (`--only-binary=:all:` / `--ignore-scripts`, so an install script never runs). That command is then substituted out of the line — with `true` when it worked and `false` when it did not — so the operators around it keep exactly the meaning the model wrote: `pip install x && python y.py` installs and then runs `y.py` with no network, and does not run it at all if the install failed, while `pip install x || pip install y` still tries the fallback. The failure is reported either way. Handing the install command a socket instead was the earlier design and could not be made to hold: the line is written by the model, so `--index-url` pointed the installer wherever it liked, and the socket belonged to the whole line, so everything sharing it with the install shared its reach — on a host whose sandbox cannot pin egress to a proxy, that reach was the open internet. So an option that would change where a package comes from (`--index-url`, `-i`, `--find-links`, `--registry`, a requirement that is itself a URL) is refused by name, as is an installer the host cannot perform on your behalf (`uv`, `poetry`, `gem`, `cargo`, `go`); `-r requirements.txt` is honoured by reading the file and validating every line. Env: `TS_CODE_EXEC_ALLOW_INSTALL`. |
| `--code-exec-packages <list>` | Restrict installs to these package names, comma-separated; anything else is refused and the model is told which names are allowed (default: empty, meaning any package — and at most 16 packages in one install either way). Matching is on the bare name, so a version the model pins (`numpy==2.1.0`) still matches the entry `numpy`. It was retired when the tool surface became a shell, because a model typing its own `pip install` could spell the request in ways a name list could not see; it is back because the **host** performs every install again — it reads the names out of the model's command rather than running that command, and builds the installer's argument vector itself, so the list applies however the request was spelled (`pip`, `pip3`, `python -m pip`, a requirements file). Only meaningful with `--code-exec-allow-install`. |
| `--code-exec-install-domains <list>` | The hosts an install may reach, comma-separated — exact names, or `*.suffix` wildcards (default: `pypi.org,files.pythonhosted.org,registry.npmjs.org`; an empty value disables the pinning and restores an open install network). It bounds the install the **host** performs, which is the only thing here that reaches a registry at all: where the sandbox can pin a single loopback port (macOS Seatbelt) the installer talks to a CONNECT egress proxy and so reaches these hosts and nothing else, and that proxy is created for one install and closed with it, so a child the installer left running finds nothing on the port to talk to. Where the sandbox cannot pin a port (bubblewrap is all-or-nothing about the network) the installer gets a real network, and what holds there is that the argument vector is the host's rather than the model's — the default index, wheels only. Exact names rather than wildcards by default, because `*.pypi.org` would also admit `upload.pypi.org`. A host the proxy refused is named in the failure the model reads, because pip reports it only as an opaque connection error. Env: `TS_CODE_EXEC_INSTALL_DOMAINS`. |
...
| `--code-exec-unconfined` | Run the model's commands even where the OS cannot confine them — the sandbox is otherwise required and the tool refuses on a host that cannot restrict files and sockets, which on Windows is every host (a job object bounds CPU and memory but neither of those), so this is the only way to use the tool there. It means model-written commands run with your account's access to the filesystem and the network. Deliberately **CLI-only**: the server refuses this flag at startup (exit code 2), because its operator cannot make that trade on behalf of everyone who can reach the port. |
...
host's, not the model's, and it is unconditional: a command the model wrote has
**no network**, on every platform and in every configuration. Installs are the
only thing that reaches a registry, and the host performs those itself — the
...
| `--spec-pmin <f>` | Draft-confidence gate in `[0, 1]`; drafting stops at the first token below it, and `0` means never gate. What the number MEANS is the algorithm's business, so each brings its own default: `0.75` for a per-token head (top-1 probability over its top-10 logits), `0.35` for a block drafter (the CUMULATIVE prefix probability — the product of the confidence head's per-position estimates, so the same number is far stricter; lower drafts further and rolls back more, higher falls back to plain decode more often), `0` for n-gram (where it scales the required match length instead). Env: `TS_SPEC_PMIN` (or `TS_MTP_PMIN`). |
...
| `--skills-sandbox <off\|preferred\|required>` | How hard to insist on OS isolation for a skill's scripts. `required` (the default) refuses to run them on a host with no sandbox rather than running them unconfined; `preferred` runs them anyway; `off` applies only the in-process limits. macOS uses `sandbox-exec`, Linux uses `bwrap`, Windows bounds the process tree only. Env: `TS_SKILLS_SANDBOX`. |
| `--skills-allow-network` | Let a sandboxed skill script reach the network. Denied by default. Env: `TS_SKILLS_ALLOW_NETWORK`. |
| `--code-exec` | Offer the model a `shell` tool: it types a real command line, the host runs it in an OS sandbox, and the model reads back the exit code and everything the command printed, stdout and stderr merged. This is how the model RUNS things and looks around — run a program, `rg` for a pattern, install what it needs, check its own output — which is the shape OpenAI Codex and Claude Code use. Reading and changing files is the file tools' job, below. Arguments: `command` (required), `workdir` (that one call only), `timeout_ms`, and `run_in_background` for something meant to keep running, whose result names a log file to read later. Files a command writes are handed to the user as download links, and the working directory persists for the rest of that chat session. Four more tools come with it, and they are where file work actually happens. `read_file` (`path`, `offset`, `limit`) shows a file's current bytes with line numbers rendered `   42 \| text`, and re-reading an unchanged file says so instead of repeating it. `edit_file` (`path`, `old_string`, `new_string`, `replace_all`) replaces one exact string in one file — Claude Code's `Edit`, parameter for parameter, and the shape Anthropic publishes as `str_replace_based_edit_tool`. It must match exactly and exactly once; if it matches twice the call is refused and the matches are located for you, and if it matches nothing you are shown the file's real numbered lines around the closest thing to what you asked for. `write_file` (`path`, `content`) creates a file or deliberately replaces one whole — and when it replaces one, the host counts how many of the lines came back byte-identical and says so. `apply_patch`, Codex's `*** Begin Patch` envelope, creates, updates, deletes and renames several files in one all-or-nothing call, and is reachable two ways — as a tool call, and by typing it into the shell as a heredoc, which the host intercepts and never executes. The split follows the two references rather than inventing a format: string replacement for the common one-file change, an atomic envelope for the multi-file one. All of them exist beside a shell that could rewrite any file with a heredoc because a heredoc re-emits the *whole* file: a three-line fix costs every already-correct line and re-rolls each of them. And because the **host** places the bytes, from text it either finds or refuses to guess at, rather than the model retyping a file it half-remembers. `apply_patch`'s matcher is a line-for-line port of the reference V4A applier: exact match, else trailing-whitespace-insensitive, else leading-and-trailing-insensitive, else fail — no similarity scoring, no nearest match. `edit_file`'s ladder is the union of what both references tolerate — exact, then typographic quotes and dashes folded to their ASCII forms, then a literal `\uXXXX` decoded, then line-number prefixes stripped — every rung above the first reported on the result, and the replacement written back in the file's own punctuation so tolerance never silently changes bytes nobody asked about. The four file tools need the persistent workspace, so the stateless endpoints get the shell alone. Every tool is answered in process; none is ever handed back to the client. The sandbox is **required**: on a host that cannot confine a process the tool refuses rather than running unconfined, and there is no override here (`--code-exec-unconfined` is CLI-only; this server rejects it at startup with exit code 2), because an operator cannot make that trade for everyone who can reach the port. Both tools are answered in process - neither is ever handed back to the client. Off by default. Env: `TS_CODE_EXEC` (anything but `0` counts as on). |
| `--code-exec-allow-install` | Let the model install the packages it needs (pip / npm) into an environment the chat session keeps, so a later command — and a skill's own scripts — can import them too; requires `--code-exec`. The command the model typed still gets **no network**: the host does not run its install, it *reads* it — the tool and the package names, nothing else — checks the names, and performs the install itself with an argument vector it built, wheels only (`--only-binary=:all:` / `--ignore-scripts`, so an install script never runs). That command is then substituted out of the line — with `true` when it worked and `false` when it did not — so the operators around it keep exactly the meaning the model wrote: `pip install x && python y.py` installs and then runs `y.py` with no network, and does not run it at all if the install failed, while `pip install x || pip install y` still tries the fallback. The failure is reported either way. Handing the install command a socket instead was the earlier design and could not be made to hold: the line is written by the model, so `--index-url` pointed the installer wherever it liked, and the socket belonged to the whole line, so everything sharing it with the install shared its reach — on a host whose sandbox cannot pin egress to a proxy, that reach was the open internet. So an option that would change where a package comes from (`--index-url`, `-i`, `--find-links`, `--registry`, a requirement that is itself a URL) is refused by name, as is an installer the host cannot perform on your behalf (`uv`, `poetry`, `gem`, `cargo`, `go`); `-r requirements.txt` is honoured by reading the file and validating every line. Env: `TS_CODE_EXEC_ALLOW_INSTALL`. |
| `--code-exec-packages <list>` | Restrict installs to these package names, comma-separated; anything else is refused and the model is told which names are allowed (default: empty, meaning any package — and at most 16 packages in one install either way). Matching is on the bare name, so a version the model pins (`numpy==2.1.0`) still matches the entry `numpy`. It was retired when the tool surface became a shell, because a model typing its own `pip install` could spell the request in ways a name list could not see; it is back because the **host** performs every install again — it reads the names out of the model's command rather than running that command, and builds the installer's argument vector itself, so the list applies however the request was spelled (`pip`, `pip3`, `python -m pip`, a requirements file). Only meaningful with `--code-exec-allow-install`. |
| `--code-exec-install-domains <list>` | The hosts an install may reach, comma-separated — exact names, or `*.suffix` wildcards (default: `pypi.org,files.pythonhosted.org,registry.npmjs.org`; an empty value disables the pinning and restores an open install network). It bounds the install the **host** performs, which is the only thing here that reaches a registry at all: where the sandbox can pin a single loopback port (macOS Seatbelt) the installer talks to a CONNECT egress proxy and so reaches these hosts and nothing else, and that proxy is created for one install and closed with it, so a child the installer left running finds nothing on the port to talk to. Where the sandbox cannot pin a port (bubblewrap is all-or-nothing about the network) the installer gets a real network, and what holds there is that the argument vector is the host's rather than the model's — the default index, wheels only. Exact names rather than wildcards by default, because `*.pypi.org` would also admit `upload.pypi.org`. A host the proxy refused is named in the failure the model reads, because pip reports it only as an opaque connection error. Env: `TS_CODE_EXEC_INSTALL_DOMAINS`. |
| `--code-exec-timeout <seconds>` | How long one command may run before it is killed (default `120`, up from the 30 s the program runner used, because a shell command is also how packages are installed and how a build or a test suite is run). A call may ask for less or more through `timeout_ms`, up to 10 minutes; a command that runs over is stopped and the model still gets everything it printed first, because a timeout that discards the output only makes it run the command again blind. |
| `--code-exec-shell <path\|name>` | Which shell to run commands through, when the host's own choice is wrong or absent. Default: `bash`, then `sh`, on macOS and Linux; PowerShell 7 (`pwsh`), then Windows PowerShell, on Windows — where a bare `bash` on PATH is refused on purpose, because it is the WSL launcher and running through it would put the command inside a Linux VM while the job object holds only the launcher on the Windows side. Point this at a real bash (Git Bash, MSYS2) to use one there. The model is told which dialect it is typing into, so this also changes the examples in its tool description. |
...
belongs to the host, not to the model, and it is unconditional: a command the
model wrote has **no network**, on every platform and in every configuration.
Installs are the only thing that reaches a registry, and the host performs
those itself — it reads the tool and the package names out of each recognised
install, runs the installer with an argument vector it built, and substitutes
...
after a `&&` runs, and a `||` fallback does. The failure is reported either way. Giving the
install command its own socket was the earlier design, and it had two holes
...
| `TS_SPEC_PMIN` *(legacy `TS_MTP_PMIN`)* | Draft-confidence gate in `[0, 1]`, `0` = never gate (default per algorithm: `0.75` per-token head, `0.35` block, `0` n-gram). CLI: `--spec-pmin`. |
...
| OS sandbox | `required` (refuse where files and sockets cannot be confined) | — | `--code-exec-unconfined` (CLI-only; the server refuses it at startup) |

`TS_CODE_EXEC` and `TS_CODE_EXEC_ALLOW_INSTALL` treat any value other than `0` as
on. A command the model wrote never gets a socket, on any host and in any
configuration; the only thing that reaches a registry is the install the **host**
performs on its behalf, from the package names it read out of that command.
