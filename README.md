...
- **🖥️ Sandboxed code execution.** With `--code-exec` the model drives a real shell: it types a command line, the host runs it inside an OS sandbox, and it reads back the exit code and everything the command printed — write a file with a heredoc, run it, grep it, install what it needs, check its own output. The working directory persists for the whole chat session and is shared with skill scripts, so `cd`, exported variables, an activated virtualenv and installed packages all survive from call to call. File edits go through `apply_patch`, an all-or-nothing envelope that creates, updates, deletes and renames several files in one call: the **host** places the bytes, from anchors it either finds or refuses to guess at, rather than the model retyping a file it half-remembers. A command the model wrote never reaches the network, on any host and in any configuration: installing works because the **host** reads the package names out of the command and performs the install itself, wheels only and against its own registry allow-list. Off by default, and the sandbox is not optional — on a host that cannot confine a process the tool refuses rather than running unconfined (`--code-exec-unconfined` relaxes that on the CLI only; the server rejects it at startup). → [Usage](USAGE.md)
