...
- **🧩 Agent Skills.** Point the CLI or the server at a folder of skills — a `SKILL.md` of model-facing instructions plus the scripts, references and assets it needs — and select them per request with `"skills": ["pdf"]` on any chat API or `--skill` on the CLI. Only the one-line descriptions cost context up front; the model pulls instructions and reference files on demand through built-in `skills_list` / `skills_read` tools that **TensorSharp executes itself, in process**, so an ordinary OpenAI client never sees a tool call it cannot service. The block injected into the prompt is a pure function of the sorted selection, so the KV prefix cache keeps matching turn to turn; every path a model names is confined to that skill's own directory, and running a skill's scripts stays off unless `--skills-allow-exec` is passed. Loads the [published open-source skills](https://github.com/anthropics/skills) unmodified. → [Agent Skills](docs/agent_skills.md)
- **🖥️ Sandboxed code execution.** With `--code-exec` the model gets a patch-first coding surface: `read_file` for bounded source context, exact-match `edit_file` for a local fix, `write_file` only for a new or intentionally replaced file, atomic multi-file `apply_patch`, and `shell` for running tests and commands. When a diagnostic identifies a bounded regular workspace source file, a failed program points back to its bounded source excerpt and tells the model to edit the smallest faulty region, then rerun the check—so a one-line bug does not resend and re-roll every correct line. The working directory persists for the whole Web/CLI chat; each OpenAI/Ollama HTTP request gets its own isolated workspace across all internal repair rounds, deleted when that request finishes, so separate requests remain stateless. When code execution is enabled, skill scripts share the same request or chat workspace; script-only execution retains its per-call scratch directory. Internet/IP access is denied by default; `--code-exec-allow-network` (or non-zero `TS_CODE_EXEC_ALLOW_NETWORK`) is the explicit opt-in that gives every model-authored command unrestricted host IP-network access while retaining the macOS/Linux workspace-write and home-read boundaries (with the documented macOS shared-temp exception). Linux additionally bounds descendants with a PID namespace. On macOS, children inherit Seatbelt and ordinary process groups are stopped, but a deliberately detached child can outlive the request; every result reports that gap. That includes LAN/loopback services and IP listening sockets, so generated code can exfiltrate any other host-readable data. macOS denies common `/private/tmp/com.apple.launchd*` pathname sockets while permitting runtime-required Mach lookup and the exact mDNSResponder pathname socket required for DNS, and Linux hides common `/run` endpoints, but local Unix IPC is not a complete isolation boundary: macOS retains shared-temporary-directory Unix IPC for compatibility, and Linux's host network namespace may expose abstract sockets and pathname sockets outside `/run`. Enable network access only for trusted tasks. It is separate from `--skills-allow-network`, which controls bundled skill scripts. Package installs remain a separate host-performed capability, but their package/domain allow-lists cannot constrain direct downloads after unrestricted networking is enabled. Code execution itself is off by default, and the sandbox is not optional — on a host that cannot confine a process the tool refuses rather than running unconfined. Linux requires `bwrap` 0.12.0 or newer; Windows requires `--code-exec-unconfined`, accepted by both the CLI and server, and that intentionally leaves filesystem and network access unconfined. → [Usage](USAGE.md)
...
`ggml_vulkan`). On architectures that shard no weights — Qwen 3.8 Flash Next,
DeepSeek V4, GLM 5.x — the same flag runs a layer split instead: one
contiguous run of whole layers per GPU. Install the CUDA toolkit first, then:
...
| Qwen 3.8 Flash Next | `qwen4exp` | Qwen3.8-Flash-Next (hybrid MoE, 512 experts / 10 used, GatedDeltaNet on 36 of 48 layers interleaved with QSA-indexed full attention, PLE n-gram block, ×4 hyper-connections) | Image | Yes | Yes | — | [qwen38-flash-next.md](docs/models/qwen38-flash-next.md) |
...
| [Agent Skills](docs/agent_skills.md) | The `SKILL.md` format, progressive disclosure and its budget, the in-process tool loop, the path/ZIP/exec security model, and the HTTP + C# surfaces |
...
| Agent Skills | Skill directories discovered from `--skills-dir` (or a `skills` folder beside the binary) and installable at runtime through `POST /api/skills`. Selected per request with `"skills": [...]` on `/v1/chat/completions`, `/v1/responses`, `/api/chat` (Ollama) and `/api/chat` (Web UI), or with `--skill` on the CLI. Progressive disclosure through built-in `skills_list` / `skills_read` tools answered in process, so unmodified OpenAI clients get a finished completion; the caller's own tools are still returned to it. Script execution (`skills_run`) is off unless `--skills-allow-exec` is passed. Mistral 3 carries no tool declarations, so skill bodies are written into its prompts up front instead. |
